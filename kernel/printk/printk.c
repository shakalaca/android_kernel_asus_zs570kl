...
void resume_console(void)
{
	...
	if (pm_pwrcs_ret)
	{
		...
		printk(KERN_EMERG "[PM] Suspended for %d.%02d secs \n", pwrcs_time/100, pwrcs_time%100);
#endif
		if (gpio_irq_cnt>0)
		{
			gpio_irq_cnt = gpio_irq_cnt>7?7:gpio_irq_cnt;
			for (i=0;i<gpio_irq_cnt;i++)
				printk(KERN_EMERG "[PM] GPIO triggered: %d\n", gpio_resume_irq[i]);
			gpio_irq_cnt=0; //clear log count
		}
		if (gic_irq_cnt>0)
		{
			gic_irq_cnt = gic_irq_cnt>7?7:gpio_irq_cnt;
			for (i=0;i<gic_irq_cnt;i++)
				printk(KERN_EMERG "[PM] IRQs triggered: %d\n", gic_resume_irq[i]);

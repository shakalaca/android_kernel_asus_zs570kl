                           ...
                           struct pn5xx_i2c_platform_data *pdata)
{
    ...
    val = of_get_named_gpio_flags(node, "qcom,nq-ven", 0, &flags);
    if (val >= 0) {
        pdata->ven_gpio = val;
    }
    else{
    ...
    val = of_get_named_gpio_flags(node, "qcom,nq-firm", 0, &flags);
    if (val >= 0) {
        pdata->firm_gpio = val;
    }
    else {
    ...
    val = of_get_named_gpio_flags(node, "qcom,nq-irq", 0, &flags);
    if (val >= 0) {
        pdata->irq_gpio = val;
    }
    else {
    ...
    val = of_get_named_gpio_flags(node, "qcom,nq-clkreq", 0, &flags);
    if (val >= 0) {

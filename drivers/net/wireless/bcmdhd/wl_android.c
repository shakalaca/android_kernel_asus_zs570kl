...
#define CMD_ASSOCPREFER	"ASSOCPREFER"
...
wl_android_set_sarlimit_txctrl(struct net_device *dev, const char* string_num)
{
	 ...
	 */
	if (mode == 0) {
		DHD_ERROR(("%s: SAR limit control activated\n", __FUNCTION__));
		setval = 1;
	} else if (mode == -1) {
		DHD_ERROR(("%s: SAR limit control deactivated\n", __FUNCTION__));
		setval = 0;
	} else {
		return -EINVAL;
	}
...
#if defined(DHD_ASSOC_PREFER)
int wl_android_set_assocprefer(struct net_device *dev, char *command, int total_len)
{
	int error = 0;
	uint32 mode = 0;
#ifdef WL_CFG80211
	...
	dhdp->assoc_prefer = mode;
	error = wldev_ioctl(dev, WLC_SET_ASSOC_PREFER, &mode, sizeof(mode), true);
	if (error) {
		DHD_ERROR(("%s: failed to set assoc prefer %d, error = %d\n",
		__FUNCTION__, mode, error));
		return -1;
	}
	else
		DHD_ERROR(("%s: succeeded to set assoc prefer %d, error = %d\n",
		__FUNCTION__, mode, error));
	return 0;
}
#endif /* DHD_ASSOC_PREFER */
...
#endif /* BCMFW_ROAM_ENABLE */
#if defined(DHD_ASSOC_PREFER)
	else if (strnicmp(command, CMD_ASSOCPREFER, strlen(CMD_ASSOCPREFER)) == 0) {
		bytes_written = wl_android_set_assocprefer(net, command, priv_cmd.total_len);
	}
#endif /* DHD_ASSOC_PREFER */

 ...
 *
 * $Id: wl_cfgvendor.c 650650 2016-07-22 05:22:05Z $
 */
...
#if defined(CUSTOMER_HW_ZEN)
static int wl_cfgvendor_interface_create(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	int err = BCME_ERROR, rem, type;
	const struct nlattr *iter;
	...
	nla_for_each_attr(iter, data, len, rem) {
		...
		switch (type) {
			case ANDR_WIFI_ATTRIBUTE_INTERFACE_CREATE:
	...
	msleep(150);
	err = wl_cfg80211_interface_create(wdev->netdev, ifname);
	if (err < 0) {
...
static const struct wiphy_vendor_command wl_vendor_cmds [] = {
...
#ifdef CUSTOMER_HW_ZEN
	{
		{
			.vendor_id = OUI_GOOGLE,
			.subcmd = ANDR_WIFI_INTERFACE_CREATE
		},
		.flags = WIPHY_VENDOR_CMD_NEED_WDEV | WIPHY_VENDOR_CMD_NEED_NETDEV,

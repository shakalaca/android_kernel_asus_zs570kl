 ...
 *
 * $Id: dhd_pno.c 633634 2016-04-25 01:47:50Z $
 */
...
_dhd_pno_set(dhd_pub_t *dhd, const dhd_pno_params_t *pno_params, dhd_pno_mode_t mode)
{
	...
	NULL_CHECK(dhd->pno_state, "pno_state is NULL", err);
	_pno_state = PNO_GET_PNOSTATE(dhd);
	DHD_PNO(("%s: _pno_state->pno_mode = %d\n", __FUNCTION__, _pno_state->pno_mode));
	...
	if (mode & DHD_PNO_GSCAN_MODE) {
		...
		if (pno_params->params_gscan.mscan) {
			...
			pfn_param.flags |= (ENABLE << ENABLE_BD_SCAN_BIT);
			DHD_PNO(("%s: pfn_param.bestn = %d, pfn_param.mscan = %d\n", __FUNCTION__, pfn_param.bestn, pfn_param.mscan));
		}
		/* RSSI margin of 30 dBm */
		pfn_param.rssi_margin = htod16(PNO_RSSI_MARGIN_DBM);
		/* ADAPTIVE turned off */
         ...
         void *info, uint32 *len)
{
	...
	switch (type) {
		case DHD_PNO_GET_CAPABILITIES:
			...
			//test gscan hugh 2016-0624 ++++
			ptr->max_ap_cache_per_scan = GSCAN_MAX_AP_CACHE_PER_SCAN;//16;
			//test gscan hugh 2016-0624 ----
...
dhd_pno_set_for_gscan(dhd_pub_t *dhd, struct dhd_pno_gscan_params *gscan_params)
{
	...
	DHD_PNO(("%s: gscan_param_size = %d, num_buckets_to_fw = %d\n",__FUNCTION__, gscan_param_size, num_buckets_to_fw));

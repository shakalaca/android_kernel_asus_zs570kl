...
//		sleep_on_timeout(&tput_wait_queue, tput_check_interval_s * HZ);//sleep tput_check_interval_s
        wait_event_timeout(tput_wait_queue, false, tput_check_interval_s * HZ);
	}
	DHD_ERROR(("%s: end\n",  __FUNCTION__));
	return 0;
}
...
uint disable_random_mac_softap = 0;
module_param(disable_random_mac_softap, uint, 0644);
...
dhd_tput_monitor_thread(void *data)
{
	...
	while (1) {
		if (down_interruptible(&tsk->sema) == 0) {
            ...
            if (ifp && (passed_msec >= ZEN_TPUT_CHECK_INTERVAL * 1000)) {
                pre_jiffies = jiffies;
            ...
            if (flag)
                mod_timer(&dhd->tput_timer,
                        jiffies + msecs_to_jiffies(ZEN_TPUT_CHECK_INTERVAL * 1000));
		} else {
			break;
		}
	}
...
dhd_tput_monitor_func(ulong data)
{
    ...
    if (dhd && dhd->tput_tsk.thr_pid >= 0) {
        if (time_after_eq(jiffies,
                    dhd->last_jiffies + msecs_to_jiffies(ZEN_TPUT_CHECK_INTERVAL * 1000)) ||
...
dhd_tput_monitor_init(dhd_pub_t *dhdp)
{
    ...
    mod_timer(&dhd->tput_timer, jiffies + msecs_to_jiffies(ZEN_TPUT_CHECK_INTERVAL * 1000));
}
	...
	if (dhd->up) {
		if (value && dhd->in_suspend) {
			...
			if (dhd->info->shub_enable == 1) {
				ret = dhd_iovar(dhd, 0, "shub_msreq", (char *)&shub_msreq, sizeof(shub_msreq), NULL, 0, TRUE);
				if (ret < 0) {
				...
				for (i = 0; i < DHD_MAX_IFS; i++) {
					if (dhdinfo->iflist[i] && dhdinfo->iflist[i]->net) {
						dhd_iovar(dhd, i, "allmulti", (char *)&allmulti, sizeof(allmulti),
						NULL, 0, TRUE);
					}
				}
#endif /* PASS_ALL_MCAST_PKTS */
				...
				dhd_iovar(dhd, 0, "roam_time_thresh", (char *)&roam_time_thresh, sizeof(roam_time_thresh), NULL, 0, TRUE);
#endif /* CUSTOM_ROAM_TIME_THRESH_IN_SUSPEND */
#ifndef ENABLE_FW_ROAM_SUSPEND
				...
				shub_msreq = 0;
				if (dhd->info->shub_enable == 1) {
					ret = dhd_iovar(dhd, 0, "shub_msreq", (char *)&shub_msreq,
						sizeof(shub_msreq), NULL, 0, TRUE);
					if (ret < 0) {
						DHD_ERROR(("%s Sensor Hub move/stop stop: failed %d\n",
							__FUNCTION__, ret));
					}
				}
#endif /* SUPPORT_SENSORHUB */
...
#ifdef DHD_ASSOC_PREFER
	dhd->pub.assoc_prefer = WLC_BAND_AUTO;
#endif /* DHD_ASSOC_PREFER */
...
#ifdef CUSTOMER_HW_ZEN
	/* init interrupt at cpu 0 for power saving */
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#if defined(DISABLE_11AC) || defined(CUSTOMER_HW_ZEN)
	uint32 vhtmode = 0;
#endif /* DISABLE_11AC || CUSTOMER_HW_ZEN */
#ifdef CUSTOMER_HW_ZEN
    uint maxassoc = 0;
#endif /* CUSTOMER_HW_ZEN */
#ifdef USE_WL_TXBF
...
#ifdef DISABLE_SCAN_PARALLEL
	uint32 scan_parallel = 0;
#endif /* DISABLE_SCAN_PARALLEL */
	...
	if (!ret) {
		...
		ret = dhd_iovar(dhd, 0, "cur_etheraddr", NULL, 0, (char *)&ea_addr, ETHER_ADDR_LEN, 
				FALSE);
		if (ret < 0) {
...
#ifdef SET_RANDOM_MAC_SOFTAP
#ifdef CUSTOMER_HW_ZEN
	if (disable_random_mac_softap == 0) {
#endif /* CUSTOMER_HW_ZEN */
	...
	if (FW_SUPPORTED(dhd, rsdb)) {
		...
		ret = dhd_iovar(dhd, 0, "scan_features", (char *)&scan_features, sizeof(scan_features),
	...
	if (dhd->op_mode == DHD_FLAG_HOSTAP_MODE) {
		...
		if (ret < 0)
		{
			DHD_ERROR(("%s vhtmode set failed %d\n", __FUNCTION__, ret));
			if (ret == BCME_NOTDOWN) {
				uint wl_down = 1;
				ret = dhd_wl_ioctl_cmd(dhd, WLC_DOWN,
					(char *)&wl_down, sizeof(wl_down), TRUE, 0);
				DHD_ERROR(("%s vhtmode fail WL_DOWN : %d, vhtmode = %d\n",
					__FUNCTION__, ret, vhtmode));

				ret = dhd_iovar(dhd, 0, "vhtmode", (char *)&vhtmode,
					sizeof(vhtmode), NULL, 0, TRUE);
				DHD_ERROR(("%s vhtmode set. ret --> %d\n", __FUNCTION__, ret));
			}
		}

        maxassoc = MAX_ASSOC_NUMBER;
		ret = dhd_iovar(dhd, 0, "maxassoc", (char *)&maxassoc, sizeof(maxassoc), NULL, 0, TRUE);
		if (ret < 0) {
            DHD_ERROR(("%s set maxassoc failed %d\n", __FUNCTION__, ret));
        }
	}
#else
	ret = dhd_iovar(dhd, 0, "vhtmode", (char *)&vhtmode, sizeof(vhtmode), NULL, 0, TRUE);
...
#ifdef DHD_ASSOC_PREFER
	DHD_ERROR(("%s: set assoc prefer %d\n", __FUNCTION__, dhd->assoc_prefer));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ASSOC_PREFER, &dhd->assoc_prefer,
		sizeof(dhd->assoc_prefer), TRUE, 0)) < 0)
		DHD_ERROR(("%s: assoc prefer set failed %d\n", __FUNCTION__, ret));
#endif /* DHD_ASSOC_PREFER */
#endif /* ROAM_ENABLE */
	...
	/* Setup srl value */
	dhd_iovar(dhd, 0, "srl", (char *)&srl_max, sizeof(srl_max), NULL, 0, TRUE);

	/* Setup lrl value */
	dhd_iovar(dhd, 0, "lrl", (char *)&lrl_max, sizeof(lrl_max), NULL, 0, TRUE);
#if defined(AP) && !defined(WLP2P)
	/* Turn off MPC in AP mode */
...
#ifdef CUSTOM_PSPRETEND_THR
	/* Turn off MPC in AP mode */
	ret = dhd_iovar(dhd, 0, "pspretend_threshold", (char *)&pspretend_thr, sizeof(pspretend_thr), NULL, 0, TRUE);
	if (ret < 0) {
...
#elif defined(SHOW_LOGTRACE) && !defined(CUSTOMER_HW_ZEN)
	setbit(eventmask, WLC_E_TRACE);
#else
	clrbit(eventmask, WLC_E_TRACE);
...
#ifdef SUPPORT_SENSORHUB
	ret = dhd_iovar(dhd, 0, "shub", NULL, 0,
			(char *)&shub_enable, sizeof(shub_enable), TRUE);
	if (ret < 0) {
...
//	wl_ioctl_t ioc;
	...
	if (ifidx == 0) {
...
#if defined(WL_VIRTUAL_APSTA) && defined(CUSTOMER_HW_ZEN)
		...
		ret = dhd_iovar(&dhd->pub, 0, "maxassoc", (char *)&maxassoc, sizeof(maxassoc),
				NULL, 0, TRUE);
		if (ret < 0) {
			DHD_ERROR(("%s: set maxassoc failed\n", dhd_ifname(&dhd->pub, ifidx)));
		}
#endif /*  defined(WL_VIRTUAL_APSTA) && defined(CUSTOMER_HW_ZEN) */
		/*
...
#if defined(DHD_OF_SUPPORT) && defined(CUSTOMER_HW_ZEN)
	dhd_wlan_exit();
#endif /* defined(DHD_OF_SUPPORT) && defined(CUSTOMER_HW_ZEN) */
}
        ...
        if (dhdp) {
            if (!dhdp->dongle_trap_occured) {
                ...
                if (g_wifi_on) {
                    ...
                    DHD_ERROR(("%s: power off already! skip pcie recovery.\n",
                                __FUNCTION__));
                }
...
#endif /* DHD_OF_SUPPORT && !CUSTOMER_HW_ZEN */

...
#ifdef CUSTOMER_HW_ZEN
    primary_ndev = bcmcfg_to_prmry_ndev(cfg);
    WL_ERR(("cfg=%p, primary_ndev=%p, primary_ifname=%s dev=%p dev_ifname=%s\n",
                cfg, primary_ndev, primary_ndev->name, dev, dev->name));
...
#define SCAN_EBUSY_RETRY_LIMIT 20
	if (err == -EBUSY) {
		if (busy_count++ > SCAN_EBUSY_RETRY_LIMIT) {
			...
			*/
#ifdef CUSTOMER_HW_ZEN
            if (busy_count == (SCAN_EBUSY_RETRY_LIMIT/2)) {
                WL_ERR((" %s ESCAN BUSY 10 times, issue SCAN_ABORT\n",
                            __FUNCTION__));
                wl_cfg80211_scan_abort(cfg);
            }
#endif /* CUSTOMER_HW_ZEN */
	...
	for_each_ndev(cfg, iter, next) {
		if (iter->ndev) {
			if (strcmp(iter->ndev->name, name) == 0) {
				WL_ERR(("Iface %s already created\n", name));
				return BCME_OK;
			}
		}
	}
...
#ifdef CUSTOMER_HW_ZEN
    OSL_SLEEP(1000);
    err = wldev_ioctl_set(dev, WLC_INIT, &reinit, sizeof(s32));
...
#ifdef CUSTOMER_HW_ZEN
	wdev->wiphy->flags |= WIPHY_FLAG_DFS_OFFLOAD;
#endif /* CUSTOMER_HW_ZEN */
#else

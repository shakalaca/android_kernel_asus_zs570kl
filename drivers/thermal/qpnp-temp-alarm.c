...
//CPU0
ssize_t fac_thermal_cpu0_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("tsens_tz_sensor4");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
        }

        printk(KERN_EMERG "[FAC]%s:thermal dev name = %s\n", __func__, tzd->type);
	thermal_zone_get_temp(tzd, &temp);
	temp *= 100;
	len = sprintf(kernelbuf,"%ld\n",temp);
	return simple_read_from_buffer(userbuf, size,loff_p,kernelbuf,len);
}
struct file_operations fac_thermal_cpu0_fops = {
	.read=fac_thermal_cpu0_read,
};
//CPU1
ssize_t fac_thermal_cpu1_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("tsens_tz_sensor6");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_cpu1_fops = {
	.read=fac_thermal_cpu1_read,
};
//CPU2
ssize_t fac_thermal_cpu2_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("tsens_tz_sensor9");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_cpu2_fops = {
	.read=fac_thermal_cpu2_read,
};
//CPU3
ssize_t fac_thermal_cpu3_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("tsens_tz_sensor11");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_cpu3_fops = {
	.read=fac_thermal_cpu3_read,
};
//pa_thermal_0
ssize_t fac_thermal_pa_thermal_0_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("pa_therm0");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_pa_thermal_0_fops = {
	.read=fac_thermal_pa_thermal_0_read,
};
//pm8994_tz
ssize_t fac_thermal_pm8994_tz_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("pm8994_tz");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_pm8994_tz_fops = {
	.read=fac_thermal_pm8994_tz_read,
};
//thermal_sensor1
ssize_t fac_thermal_sensor1_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("msm_therm");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_sensor1_fops = {
	.read=fac_thermal_sensor1_read,
};
//thermal_sensor2
ssize_t fac_thermal_sensor2_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("emmc_therm");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_sensor2_fops = {
	.read=fac_thermal_sensor2_read,
};
//thermal_sensor3
ssize_t fac_thermal_sensor3_read (struct file *filp, char __user *userbuf, size_t size, loff_t *loff_p)
{
	int len;
	unsigned long temp;
	char kernelbuf[64];
	...
	tzd = thermal_zone_get_zone_by_name("quiet_therm");
	if (IS_ERR(tzd))
        {
                printk(KERN_EMERG "[FAC]%s:cant get thermal zone by name!\n", __func__);
		return 0;
...
struct file_operations fac_thermal_sensor3_fops = {
	.read=fac_thermal_sensor3_read,
};
#endif
...
#if defined(ASUS_FACTORY_BUILD)
	if(proc_create("fac_thermal_cpu0", 0777, NULL, &fac_thermal_cpu0_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone5 inode is error\n");
	}
	if(proc_create("fac_thermal_cpu1", 0777, NULL, &fac_thermal_cpu1_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone7 inode is error\n");
	}
	if(proc_create("fac_thermal_cpu2", 0777, NULL, &fac_thermal_cpu2_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone10 inode is error\n");
	}
	if(proc_create("fac_thermal_cpu3", 0777, NULL, &fac_thermal_cpu3_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone12 inode is error\n");
	}
	if(proc_create("fac_thermal_pa_thermal_0", 0777, NULL, &fac_thermal_pa_thermal_0_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone25 inode is error\n");
	}
	if(proc_create("fac_thermal_pm8994_tz", 0777, NULL, &fac_thermal_pm8994_tz_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone22 inode is error\n");
	}
	if(proc_create("fac_thermal_sensor1", 0777, NULL, &fac_thermal_sensor1_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone23 inode is error\n");
	}
	if(proc_create("fac_thermal_sensor2", 0777, NULL, &fac_thermal_sensor2_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone24 inode is error\n");
	}
	if(proc_create("fac_thermal_sensor3", 0777, NULL, &fac_thermal_sensor3_fops)==NULL)
	{
		printk(KERN_ERR"create fac_thermal_zone27 inode is error\n");
	}
#endif

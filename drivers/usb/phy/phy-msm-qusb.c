		...
		unsigned long delay)
{
	...
	pr_debug("[Eye] %s: base = %p, Seq count = %d\n", __func__, base, cnt);
	for (i = 0; i < cnt; i = i+2) {
		pr_debug("[Eye] write 0x%02x to 0x%02x\n", seq[i], seq[i+1]);
...
static int qusb_phy_init(struct usb_phy *phy)
{
	...
	if (qphy->qusb_phy_init_seq)
		qusb_phy_write_seq(qphy->base, qphy->qusb_phy_init_seq,
				qphy->init_seq_len, 0);
        /* ASUS_BSP : eye diagram values +++ */
        pr_debug("[Eye] After load eye diagram device tree value +++\n");
        eye_dia_test = (u8)readl_relaxed(qphy->base + QUSB2PHY_PORT_TUNE1);
        pr_debug("[Eye] %s(): 0x%02x value = 0x%02x\n",__func__,QUSB2PHY_PORT_TUNE1,eye_dia_test);
	...
	if (qphy->tune2_efuse_reg) {
		if (!qphy->tune2_val)
                ...
                /* ASUS_BSP : Qualcomm original code change tune2 0x84 value here again +++ */
		pr_debug("[Eye] %s(): Programming TUNE2 parameter as:%x\n", __func__,
				qphy->tune2_val);
	...
	/* If tune[X] modparam set, override tune[X] value */
	if (tune1) {
		pr_info("[Eye] %s(): (modparam) TUNE1 val:0x%02x\n", __func__, tune1);
		writel_relaxed(tune1, qphy->base + QUSB2PHY_PORT_TUNE1);
	}
	if (tune2) {
		pr_info("[Eye] %s(): (modparam) TUNE2 val:0x%02x\n", __func__, tune2);
		writel_relaxed(tune2, qphy->base + QUSB2PHY_PORT_TUNE2);
	}
	if (tune3) {
		pr_info("[Eye] %s(): (modparam) TUNE3 val:0x%02x\n", __func__, tune3);
		writel_relaxed(tune3, qphy->base + QUSB2PHY_PORT_TUNE3);
	}
	if (tune4) {
		pr_info("[Eye] %s(): (modparam) TUNE4 val:0x%02x\n", __func__, tune4);
...
static int qusb_phy_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(qphy->base))
        ...
        pr_info("[Eye] qusb_phy_base, start: 0x%08X, map: %p\n", (u32)res->start, qphy->base);
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM,
							"qscratch_base");
	if (res) {

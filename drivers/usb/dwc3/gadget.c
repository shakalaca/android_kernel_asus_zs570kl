		...
		const struct dwc3_event_depevt *event, int status)
{
	...
	do {
		req = next_request(&dep->req_queued);
		if (!req) {
                        pr_info("[USB] traffic monitor fail : !req(first if)\n");
		...
		if (req->trb->ctrl & DWC3_TRB_CTRL_HWO)
                {       pr_info("[USB] traffic monitor fail : req->trb->ctrl & DWC3_TRB_CTRL_HWO(second if)\n");
			return 0;
                }
		...
		if (!(dep->flags & DWC3_EP_ENABLED)) {
                        pr_info("[USB] traffic monitor fail : !(dep->flags & DWC3_EP_ENABLED)(third if)\n");

...
struct power_supply *usb_psy_typec_core;
...
{
	//struct typec_device_ops *typec_ops = dev_get_drvdata(pdev);
    return 0;//typec_ops->i2c_status(buf);
}
...
{
	//struct typec_device_ops *typec_ops = dev_get_drvdata(pdev);
	return 0;//typec_ops->cc_status(buf);
}
...
int add_typec_device(struct device *parent, struct typec_device_ops *typec_ops)
{
	...
	usb_psy_typec_core = power_supply_get_by_name("usb");
	if (!usb_psy_typec_core) {
...
int typec_sink_detected_handler(enum typec_event typec_event)
{
	...
	power_supply_set_usb_otg(usb_psy_typec_core,

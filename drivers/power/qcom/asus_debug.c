...
static unsigned long asus_calculate_cpustat_time(struct kernel_cpustat curr_cpustat,
						struct kernel_cpustat prev_cpustat)
{
...
static void asus_kernel_top_show(struct _asus_kernel_top *ktop, int type)
{
	...
	pr_info("[K]%sCPU Usage\t\tPID\t\tName\t\tDelta\n", type == KERNEL_TOP_ACCU ? "[KTOP]" : " ");
	for (i = 0; i < NUM_BUSY_THREAD_CHECK; i++) {
...
void asus_monitor_init(void)
{
	...
	if (pm_monitor_enabled) {
		...
		asus_kernel_top_accu->prev_proc_stat = vmalloc(sizeof(long) * MAX_PID);
		asus_kernel_top_accu->curr_proc_delta = vmalloc(sizeof(long) * MAX_PID);
		...
		memset(asus_kernel_top_accu->prev_proc_stat, 0, sizeof(long) * MAX_PID);
		memset(asus_kernel_top_accu->curr_proc_delta, 0, sizeof(long) * MAX_PID);

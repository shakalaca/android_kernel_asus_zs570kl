...
struct smb1351_charger {
	...
	struct mutex		init_setting_lock;
	struct mutex		otg_setting_lock;
	struct mutex		suspend_lock;
	struct mutex		disable_dual_lock;
	struct mutex		enable_dual_lock;
					...
					bool suspend)
{
	...
	printk(KERN_EMERG "[SMB1351] %s: new suspended_status = %d\n", __func__, suspended);
...
int smb1351_init_jeita(void)
{
        ...
        printk(KERN_EMERG "[SMB1351] %s: +++\n", __func__);
        mutex_lock(&smb1351_dev->jeita_lock);
...
out:
        mutex_unlock(&smb1351_dev->jeita_lock);
        printk(KERN_EMERG "[SMB1351] %s: ---\n", __func__);
        return rc;
}
...
int smb1351_dual_enable(int flag)
{
        ...
        printk(KERN_EMERG "[SMB1351] %s: +++, flag = %d\n", __func__, flag);
        if (flag != 1 && flag != 2) {
                printk(KERN_EMERG "[SMB1351] %s: Error flag = %d\n", __func__, flag);
                return -1;
        }
        mutex_lock(&smb1351_dev->enable_dual_lock);
        //set smb1351 non-suspend
	rc = smb1351_usb_suspend(smb1351_dev, SOC, false);
	if (rc) {
        ...
        if (flag == 1) {
                //set IUSB_IN = 1000 mA
	        rc = smb1351_masked_write(smb1351_dev, 0x00, AC_INPUT_CURRENT_LIMIT_MASK,
				BIT(1));
	        if (rc) {
        ...
        //set fast charge current = 1600 mA
	rc = smb1351_masked_write(smb1351_dev, 0x00, FAST_CHG_CURRENT_MASK,
				BIT(5)|BIT(4));
	if (rc) {
...
out:
        mutex_unlock(&smb1351_dev->enable_dual_lock);
        printk(KERN_EMERG "[SMB1351] %s: ---\n", __func__);
        return rc;
}
...
int smb1351_dual_disable(void)
{
        ...
        printk(KERN_EMERG "[SMB1351] %s: +++\n", __func__);
        mutex_lock(&smb1351_dev->disable_dual_lock);
        //set smb1351 suspend
	rc = smb1351_usb_suspend(smb1351_dev, SOC, true);
	if (rc) {
	...
	if (rc) {
        ...
        mutex_unlock(&smb1351_dev->disable_dual_lock);
        printk(KERN_EMERG "[SMB1351] %s: ---\n", __func__);
...
int smb1351_set_suspend(bool suspend)
{
        ...
        printk(KERN_EMERG "[SMB1351] %s: +++\n", __func__);
        mutex_lock(&smb1351_dev->suspend_lock);
        rc = smb1351_usb_suspend(smb1351_dev, SOC, suspend);
	if (rc) {
		pr_err("Unable to suspend smb1351!\n");
	}
        mutex_unlock(&smb1351_dev->suspend_lock);
        printk(KERN_EMERG "[SMB1351] %s: ---\n", __func__);
        return rc;
...
/*
int smb1351_set_otg(int on)
{
	int rc;

        printk(KERN_EMERG "%s(%s):+++\n", __func__,on?"on":"off");
        mutex_lock(&smb1351_dev->otg_setting_lock);
	...
	if (rc) {
        ...
        //Default OTG current limit = 500mA
	rc = smb1351_masked_write(smb1351_dev, 0x0A, BIT(3)|BIT(2), BIT(2));
	if (rc) {
        ...
        if(on)
        {
        	...
        	rc = smb1351_masked_write(smb1351_dev, 0x0A, BIT(3)|BIT(2), BIT(3)|BIT(2));
	        if (rc) {
...
out:
        mutex_unlock(&smb1351_dev->otg_setting_lock);
        printk(KERN_EMERG "%s:---\n",__func__);
	return rc;
}
EXPORT_SYMBOL(smb1351_set_otg);
*/
...
int smb1351_parallel_init_setting(void)
{
	int rc;
        ...
        printk(KERN_EMERG "[SMB1351] %s: +++\n", __func__);
        mutex_lock(&smb1351_dev->init_setting_lock);
	rc = smb1351_enable_volatile_writes(smb1351_dev);
	if (rc) {
		pr_err("Couldn't configure volatile writes rc=%d\n", rc);
		goto out;
        ...
        printk("[SMB1351] %s: suspend+++\n", __func__);
	//rc = smb1351_masked_write(smb1351_dev, 0x31, BIT(6), BIT(6));
	//if (rc) {
	//	pr_err("Couldn't set charger suspend rc=%d\n", rc);
	//	return rc;
	//}
	smb1351_usb_suspend(smb1351_dev, SOC, true);
        printk("[SMB1351] %s: suspend---\n", __func__);
...
out:
        mutex_unlock(&smb1351_dev->init_setting_lock);
        printk("[SMB1351] %s: ---\n", __func__);
	return rc;
}
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		 ...
		 */
                printk("%s: POWER_SUPPLY_PROP_CHARGING_ENABLED+++\n",__func__);
		if (chip->parallel_charger_present)
			rc = smb1351_usb_suspend(chip, USER, !val->intval);
		break;
	case POWER_SUPPLY_PROP_PRESENT:
                printk("%s:POWER_SUPPLY_PROP_PRESENT+++\n",__func__);
		rc = smb1351_parallel_set_chg_present(chip, val->intval);
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
                printk("%s: POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX+++\n",__func__);
		if (chip->parallel_charger_present) {
			chip->target_fastchg_current_max_ma =
						val->intval / 1000;
			rc = smb1351_fastchg_current_set(chip,
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
                printk("%s: POWER_SUPPLY_PROP_CURRENT_MAX+++\n",__func__);
		if (chip->parallel_charger_present) {
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
                printk("%s: POWER_SUPPLY_PROP_VOLTAGE_MAX+++\n",__func__);
		if (chip->parallel_charger_present &&
	...
	case POWER_SUPPLY_PROP_USB_OTG:
		printk("[SMB1351] %s: POWER_SUPPLY_PROP_USB_OTG, enable=%d, otg_mode=%d\n",__func__, val->intval, otg_mode);
...
extern void smb1351_dump_reg(bool on)
{
	...
	if (debug_on) {
		printk(KERN_EMERG "[SMB1351]=================================================\n");
		for (addr = 0; addr <= 0x47; addr++) {
			rc = smb1351_read_reg(smb1351_dev, addr, &reg);
			if (rc)
				pr_err("Couldn't read 0x%02x rc = %d\n", addr, rc);
			else
				printk(KERN_EMERG "[SMB1351] %s: 0x%02x = 0x%02x\n", __func__, addr, reg);
		}
		printk(KERN_EMERG "[SMB1351]=================================================\n");
	} else {
...
/*+++BSP Ben proc smb1351 charger read Interface+++*/
		...
		size_t len, loff_t *data)
{
	...
	printk("[SMB1351][Proc] ChargerIC Proc File: %d\n", val);
...
//
static int smb1351_proc_read_addr = 0;
static int smb1351_reg_read_proc_read(struct seq_file *buf, void *v)
{
	int rc;
	u8 reg;
	int addr = 0;

	addr = smb1351_proc_read_addr;
	rc = smb1351_read_reg(smb1351_dev, addr, &reg);
	if (rc < 0) {
		seq_printf(buf, "Couldn't read reg 0x%X, rc=%d\n",addr, rc);
		dev_err(smb1351_dev->dev, "Couldn't read reg 0x%X, rc=%d\n",addr, rc);
		printk("Couldn't read reg 0x%X, rc=%d\n",addr, rc);
		return rc;
	}	
...
static ssize_t smb1351_reg_read_proc_write(struct file *filp, const char __user *ubuf,
		size_t len, loff_t *data)
{
	char buf[16];
	int status = len;
	...
	smb1351_proc_read_addr = myxtoi(buf);

out:
	return status;
}

static int smb1351_reg_read_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, smb1351_reg_read_proc_read, NULL);
}

static const struct file_operations smb1351_reg_read_proc_fops = {
	.owner = THIS_MODULE,
	.open =  smb1351_reg_read_proc_open,
	.write = smb1351_reg_read_proc_write,
	.read = seq_read,
	.release = single_release,
};

static void create_smb1351_reg_read_file(void)
{
	struct proc_dir_entry *smb1351_reg_read_proc_file = proc_create("driver/smb1351_read", 0666, NULL, &smb1351_reg_read_proc_fops);

	if (smb1351_reg_read_proc_file) {
		printk("[SMB1351][Proc] smb1351_reg_read create ok!\n");
	} else{
		printk("[SMB1351][Proc] smb1351_reg_read create failed!\n");
	}
	return;
}
/*---BSP Ben proc smb1351 charger read Interface---*/

/*+++BSP Ben proc smb1351 charger write Interface+++*/
static int smb1351_proc_write_var = 0;
//static int smbchg_proc_write_addr = 0;
//static char debugbuf[16] = {'\0'};
static int smb1351_reg_write_proc_read(struct seq_file *buf, void *v)
{
	int rc;
	int write_addr = smb1351_proc_read_addr;
	u8 write_var = (u8)smb1351_proc_write_var;
	...
	rc = smb1351_write_reg(smb1351_dev, write_addr, write_var);
	if (rc < 0)
	{	
		dev_err(smb1351_dev->dev, "Couldn't write reg %d rc = %d\n", write_addr, rc);
		printk("Couldn't write reg %d rc = %d\n", write_addr, rc);
		seq_printf(buf, "!!!!!!!Fail to Write address %X = 0x%2X rc = %d\n",write_addr , write_var, rc);
		return 0;
	}
...
static ssize_t smb1351_reg_write_proc_write(struct file *filp, const char __user *ubuf,
		size_t len, loff_t *data)
{
	char buf[16];
	int status = len;
	...
	//}
	//debugbuf[i] = '\0';
	smb1351_proc_write_var = myxtoi(buf);
...
static int smb1351_reg_write_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, smb1351_reg_write_proc_read, NULL);
}

static const struct file_operations smb1351_reg_write_proc_fops = {
	.owner = THIS_MODULE,
	.open =  smb1351_reg_write_proc_open,
	.write = smb1351_reg_write_proc_write,
	.read = seq_read,
	.release = single_release,
};

static void create_smb1351_reg_write_file(void)
{
	struct proc_dir_entry *smb1351_reg_write_proc_file = proc_create("driver/smb1351_write", 0666, NULL, &smb1351_reg_write_proc_fops);

	if (smb1351_reg_write_proc_file) {
		printk("[SMB1351][Proc] smb1351_reg_write create ok!\n");
	} else{
		printk("[SMB1351] smb1351_reg_write create failed!\n");
	}
	return;
}
		...
		size_t len, loff_t *data)
{
	...
	if (ubuf[0] == '1') {
		...
		printk(KERN_EMERG "[SMBCHG] g_smb1351_dump_reg_flag: true\n");
	} else if (ubuf[0] == '0') {
		g_smb1351_dump_reg_flag = false;
		printk(KERN_EMERG "[SMBCHG] g_smb1351_dump_reg_flag: false\n");
	}
...
static void create_smb1351_dump_reg_file(void)
{
	...
	if (smb1351_dump_reg_proc_file) {
		printk(KERN_EMERG "[SMB1351][Proc] smb1351_dump_reg create ok!\n");
	} else{
		printk(KERN_EMERG "[SMB1351][Proc] smb1351_dump_reg create failed!\n");
	}
	return;
}
...
static int smb1351_gpio_init(struct device_node *np)
{
        ...
        printk("[SMB1351] %s: gpio_chg2_otg_en = %d\n", __FUNCTION__, gpio_chg2_otg_en);
                if ((!gpio_is_valid(gpio_chg2_otg_en))) {
                        printk("[SMB1351] %s: gpio_chg2_otg_en is not valid!\n", __FUNCTION__);
                        return -EINVAL;
                }
        ret = gpio_request(gpio_chg2_otg_en, "chg2-otg-en-gpio");
        if (ret < 0) {
                printk("[SMB1351] %s: request chg2-otg-en gpio fail!\n", __FUNCTION__);
        }
        gpio_direction_output(gpio_chg2_otg_en, 0);
        if (ret < 0) {
                printk("[SMB1351] %s: set direction of smb1351 gpio fail!\n", __FUNCTION__);
        }
        printk("[SMB1351] %s: ---\n",__func__);
        return 0;
}
				...
				const struct i2c_device_id *id)
{
	...
	create_debugfs_entries(chip);
        create_smb1351_reg_read_file();
        create_smb1351_reg_write_file();
				...
				const struct i2c_device_id *id)
{
	...
	smb1351_dev = chip;
	mutex_init(&chip->irq_complete);
        mutex_init(&chip->init_setting_lock);
        mutex_init(&chip->otg_setting_lock);
        mutex_init(&chip->suspend_lock);
        mutex_init(&chip->disable_dual_lock);
        mutex_init(&chip->enable_dual_lock);
        mutex_init(&chip->jeita_lock);

...
#include <linux/asusdebug.h> // add for asus event log pattern
...
struct fg_chip {
	...
	bool			*batt_range_ocv;
	int			*batt_range_pct;
	/* Battery info switch name */
	struct switch_dev batt_sdev;
	/* Battery status for ASUS Event log pattern */
	struct delayed_work	batt_status_work;
};
...
#define CHECK_BATT_STATUS_S 60
static void batt_status_work(struct work_struct *work)
{
	struct fg_chip *chip = container_of(work,
				struct fg_chip,
...
static void status_change_work(struct work_struct *work)
{
	...
	if (chip->status == POWER_SUPPLY_STATUS_FULL) {
		if (capacity >= 99 && chip->hold_soc_while_full
			...
			if (fg_debug_mask & FG_STATUS)
				pr_fg(FG_STATUS, "holding soc at 100\n");
			chip->charge_full = true;
		} else if (fg_debug_mask & FG_STATUS) {
			pr_fg(FG_STATUS, "terminated charging at %d/0x%02x\n",
...
wait:
	...
	pr_fg(FG_STATUS, "v_current_pred = %d, v = %d, threshold = %d, profiles_same = %d, batt_empty = %d\n",
				fg_data[FG_DATA_CPRED_VOLTAGE].value,
...
static void charge_full_work(struct work_struct *work)
{
	...
	if (chip->status != POWER_SUPPLY_STATUS_FULL) {
		if (fg_debug_mask & FG_STATUS)
			pr_fg(FG_STATUS, "battery not full: %d\n", chip->status);
		disable = true;
	}
	...
	pr_fg(FG_STATUS, "buffer[2] = 0x%02x, resume_soc_raw = 0x%02x\n", buffer[2], resume_soc_raw);
	if (buffer[2] <= resume_soc_raw) {
		if (fg_debug_mask & FG_STATUS)
			pr_fg(FG_STATUS, "bsoc = 0x%02x <= resume = 0x%02x\n",
					buffer[2], resume_soc_raw);
		disable = true;
	}
	if (!disable)
	...
	if (fg_debug_mask & FG_STATUS) {
		...
		pr_fg(FG_STATUS, "wrote %06x into soc full\n", bsoc);
	}
	fg_mem_release(chip);
	/*
...
{
	cancel_delayed_work_sync(&chip->batt_status_work);
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->batt_status_work, batt_status_work);
	...
	/* Battery status for ASUS Event log pattern */
	schedule_delayed_work(&chip->batt_status_work,

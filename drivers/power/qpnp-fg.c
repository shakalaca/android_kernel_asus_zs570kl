				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		val->intval = get_sram_prop_now(chip, FG_DATA_CURRENT) * (-1);
		break;
...
static int gaugeIC_status_proc_read(struct seq_file *buf, void *v)
{
	...
	rc = fg_read(fg_dev, &mem_if_sts, INT_RT_STS(fg_dev->mem_base), 1);
	if (rc) {
		seq_printf(buf, "%d\n", 0);
	} else{
		seq_printf(buf, "%d\n", 1);
		...
		size_t len, loff_t *data)
{
	...
	pr_info("battery status Proc File: %d\n", val);

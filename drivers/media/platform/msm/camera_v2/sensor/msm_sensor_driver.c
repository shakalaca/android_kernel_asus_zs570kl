...
//ASUS_BSP +++ "Camera I2C Status"
//front
char g_vga_status;
static unsigned char g_vga_status_created = 0;
static void create_vga_status_proc_file(void);

//rear
char g_camera_status;
static unsigned char g_camera_status_created = 0;
static void create_camera_status_proc_file(void);
//ASUS_BSP --- "Camera I2C Status"

//ASUS_BSP +++   "implement frontCam OTP"
...
CSID_TG:
	...
	*  sensor control structure */
	...
	if (rc < 0) {
		//ASUS_BSP +++
		if (strcmp(slave_info->sensor_name, "ov8856") == 0) {
			pr_err("%s vga status failed\n",__func__);
			g_vga_status = '0';
			create_vga_status_proc_file();
		}
		else if (strcmp(slave_info->sensor_name, "imx318") == 0) {
			pr_err("%s camera status failed\n",__func__);
			g_camera_status = '0';
			create_camera_status_proc_file();
		}
		//ASUS_BSP ---
...
//ASUS_BSP +++
	if(strcmp(slave_info->sensor_name, "ov8856") == 0){
		pr_err("%s vga status succeeded\n",__func__);
		g_vga_status = '1';
                create_vga_status_proc_file();
		pr_err("%s front camera module\n",__func__);
		g_front_module = "OV8856";
		create_front_module_proc_file();
		pr_err("%s front camera resolution\n",__func__);
		g_front_resolution = "8M";
		create_front_resolution_proc_file();
		pr_err("%s front camera otp\n",__func__);
		ov8856_otp_read();
		create_front_otp_proc_file();
	}
	else if (strcmp(slave_info->sensor_name, "imx318") == 0) {
		pr_err("%s camera status succeeded\n",__func__);
		g_camera_status = '1';
                create_camera_status_proc_file();
		pr_err("%s rear camera module\n",__func__);
                g_rear_module = "IMX318";
                create_rear_module_proc_file();
		pr_err("%s rear camera resolution\n",__func__);
                g_rear_resolution = "23M";
...
static int32_t msm_sensor_driver_parse(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->msm_sensor_mutex = msm_sensor_global_mutex;
...
#define        FRONT_RESOLUTION_PROC_FILE      "driver/front_resolution"
static struct proc_dir_entry *front_resolution_proc_file;
...
static void create_front_resolution_proc_file(void)
{
       if(!g_front_resolution_created) {
           front_resolution_proc_file = proc_create(FRONT_RESOLUTION_PROC_FILE, 0666, NULL, &front_resolution_fops);
           if (front_resolution_proc_file) {
...
#define        REAR_RESOLUTION_PROC_FILE      "driver/rear_resolution"
static struct proc_dir_entry *rear_resolution_proc_file;
...
static void create_rear_resolution_proc_file(void)
{
       if(!g_rear_resolution_created) {
           rear_resolution_proc_file = proc_create(REAR_RESOLUTION_PROC_FILE, 0666, NULL, &rear_resolution_fops);
           if (rear_resolution_proc_file) {
                       CDBG("Stimber: %s sucessed!\n", __func__);
...
#define        FRONT_MODULE_PROC_FILE      "driver/front_module"
static struct proc_dir_entry *front_module_proc_file;
...
static void create_front_module_proc_file(void)
{
       if(!g_front_module_created) {
           front_module_proc_file = proc_create(FRONT_MODULE_PROC_FILE, 0666, NULL, &front_module_fops);
           if (front_module_proc_file) {
                       CDBG("Stimber: %s sucessed!\n", __func__);
...
#define        REAR_MODULE_PROC_FILE      "driver/rear_module"
static struct proc_dir_entry *rear_module_proc_file;
...
static void create_rear_module_proc_file(void)
{
       if(!g_rear_module_created) {
           rear_module_proc_file = proc_create(REAR_MODULE_PROC_FILE, 0666, NULL, &rear_module_fops);
           if (rear_module_proc_file) {
                       CDBG("Stimber: %s sucessed!\n", __func__);
...
#define        VGA_STATUS_PROC_FILE      "driver/vga_status"
static struct proc_dir_entry *vga_status_proc_file;

static int vga_status_proc_read(struct seq_file *buf, void *v)
{
    seq_printf(buf, "%c\n", g_vga_status);
    return 0;
}

static int vga_status_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, vga_status_proc_read, NULL);
}

static const struct file_operations vga_status_fops = {
       .owner = THIS_MODULE,
       .open = vga_status_proc_open,
       .read = seq_read,
       //.write = otp_proc_write,
       .llseek = seq_lseek,
       .release = single_release,
...
static void create_vga_status_proc_file(void)
{
       if(!g_vga_status_created) {
           vga_status_proc_file = proc_create(VGA_STATUS_PROC_FILE, 0666, NULL, &vga_status_fops);
           if (vga_status_proc_file) {
                       CDBG("Stimber: %s sucessed!\n", __func__);
                       g_vga_status_created = 1;
           } else {
                       printk("Stimber: %s failed!\n", __func__);
                       g_vga_status_created = 0;
           }
       } else {
        pr_info("File Exist!\n");
    }
...
#define        CAMERA_STATUS_PROC_FILE      "driver/camera_status"
static struct proc_dir_entry *camera_status_proc_file;

static int camera_status_proc_read(struct seq_file *buf, void *v)
{
    seq_printf(buf, "%c\n", g_camera_status);
    return 0;
}

static int camera_status_proc_open(struct inode *inode, struct  file *file)
{
    return single_open(file, camera_status_proc_read, NULL);
}

static const struct file_operations camera_status_fops = {
       .owner = THIS_MODULE,
       .open = camera_status_proc_open,
       .read = seq_read,
       //.write = otp_proc_write,
       .llseek = seq_lseek,
       .release = single_release,
...
static void create_camera_status_proc_file(void)
{
       if(!g_camera_status_created) {
           camera_status_proc_file = proc_create(CAMERA_STATUS_PROC_FILE, 0666, NULL, &camera_status_fops);
           if (camera_status_proc_file) {
                       CDBG("Stimber: %s sucessed!\n", __func__);
...
int __ov8856_otp_read(void)
{
	...
	//set 0x5001[3] to 0
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client, 0x5001, &temp,MSM_CAMERA_I2C_BYTE_DATA);
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client, 0x5001, (0x00&0x08)|(temp&(~0x08)),MSM_CAMERA_I2C_BYTE_DATA);
	if (rc) {
	...
	for(j = 0; j < package_num; j++) {
		for (i = 0; i < BANK_SIZE; i++) {
			s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client, 0x7010+i+j*BANK_SIZE, &bank_value[i+j*BANK_SIZE],MSM_CAMERA_I2C_BYTE_DATA);
		}
	}
	...
	//set 0x5001[3] to 1
	s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client, 0x5001, &temp,MSM_CAMERA_I2C_BYTE_DATA);
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client, 0x5001, (0x08&0x08)|(temp&(~0x08)),MSM_CAMERA_I2C_BYTE_DATA);
	if (rc) {

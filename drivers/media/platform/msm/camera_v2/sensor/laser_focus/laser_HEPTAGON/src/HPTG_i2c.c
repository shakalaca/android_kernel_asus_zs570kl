...
int Laura_device_indirect_addr_write(struct msm_laser_focus_ctrl_t *dev_t,
	...
	Laura_write_seq(sensor_i2c_client, i_reg_addr_lsb, i_reg_addr_msb, indirect_addr, register_addr, i2c_write_data, num_word, dev_t->act_device_type);
	if (status < 0) {
...
int32_t Laura_write_seq(struct msm_camera_i2c_client *client, 
							uint16_t i_reg_addr_lsb, uint16_t i_reg_addr_msb, uint16_t indirect_addr, 
							uint32_t reg_addr, uint16_t *data, uint32_t num_word, enum msm_camera_device_type_t act_device_type)
{
	int32_t rc = -EFAULT;
	uint8_t i = 0, j = 0;
	...
	int (*i2c_write) (struct msm_camera_i2c_client *, uint32_t, uint16_t,
		enum msm_camera_i2c_data_type) = client->i2c_func_tbl->i2c_write;
	...
	if (act_device_type == MSM_CAMERA_PLATFORM_DEVICE) {
		...
		for (i = 0; i < reg_conf_tbl_size; i++) {
			rc |= i2c_write(client, reg_conf_tbl[i].reg_addr, reg_conf_tbl[i].reg_data, MSM_CAMERA_I2C_BYTE_DATA);
		}
	}

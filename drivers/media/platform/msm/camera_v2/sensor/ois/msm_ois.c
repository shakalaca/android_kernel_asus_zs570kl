...
#define ASUS_OIS_SENSOR_IOC_MAGIC                      ('C')
#define ASUS_OIS_SENSOR_IOCTL_READ_DATA           _IOWR(ASUS_OIS_SENSOR_IOC_MAGIC, 1, int)

#define	ASUS_PROC_ACCEL_GAIN                       "driver/OIS_Accel_Gain"
...
static int accel_gain_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%d\n",gfctrl->ois_state);
	return 0;
}

static int accel_gain_open(struct inode *inode, struct file *file)
{
	return single_open(file, accel_gain_read, NULL);
}

static ssize_t accel_gain_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	uint32_t x_reg_addr = ADDRESS_OF_ACCEL_GAIN_X , y_reg_addr = ADDRESS_OF_ACCEL_GAIN_Y , input_data = 0;
	uint16_t accel_gain = 0;
	int32_t rc = 0;
	uint16_t reg_data = 0;
	char messages[16] = {'\0'};

	if( gfctrl == NULL  || gfctrl->ois_state != OIS_CCI_READY || is_debug == true)
		return rc;

	//printk("[8996_ois_gain]@ %zu %d \n", len, (int)len);
	...
	CDBG("[8996_ois_gain]@%s , 0x%x , 0x%x \n", messages, input_data, accel_gain);
...
static const struct file_operations accel_gain_fops = {
	.owner = THIS_MODULE,
	.open = accel_gain_open,
	.write = accel_gain_write,
	.read = seq_read,
	.llseek = seq_lseek,
	...
	void __user *argp)
{
	...
	case CFG_OIS_INIT:
	pr_err("[8996_ois]@%s\t type CFG_OIS_INIT %d\n", __func__, cdata->cfgtype);
	...
	pr_err("[8996_ois]@%s\t type CFG_OIS_I2C_WRITE_SEQ_TABLE %d\n", __func__, cdata->cfgtype);
#ifdef CONFIG_COMPAT
		if (is_compat_task()) {
			memcpy(&conf_array,
...
static int32_t msm_ois_platform_probe(struct platform_device *pdev)
{
	...
	proc_create(ASUS_PROC_ACCEL_GAIN, 0666, NULL, &accel_gain_fops);
